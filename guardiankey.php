...
class PlgAuthenticationGuardianKey extends JPlugin
{
	/**
	...
	public function onUserAuthenticate($credentials, $options, &$response)
	{
		...
		$guardiankey = new guardiankey($this->params);
		...
		if (empty($credentials['password']))
		{
			...
			//print_r($credentials).'cc';
			print_r($guadiankey->GKconfig);
			$response->error_message = $this->params;
		...
		if ($result)
		{
			...
			if ($match === true)
			{
				...
				$GKtest = json_decode($guardiankey->checkaccess($credentials['username'],$response->email,'','Authentication'));
			  if ($GKtest->response == 'ACCEPT' OR $GKtest->response == 'TIMEOUT') {
				...
				$response->status        = JAuthentication::STATUS_FAILURE;
				$response->error_message = 'Blocked by GuardianKey';
			   }
			}
			else
				{
				...
				$guardiankey->sendevent($credentials['username'],'','1');
